...
\end{eqnarray}
<<<<<<< HEAD
Here $w_0 = w_e(0)$ is the frequency observed on earth at the moment when we see the SN explosion. The first line is before we observe the SN explosion, so the only contribution to the change in the observed frequency comes from the gravitational pull to the SN. The same gravitational pull is still present in the second line, but we need to include the effect from photons crossing the neutrino shell derived in \cite{OluPie13}. Finally in the last line, the pulsar also enters the geometry inside the neutrino shell, thus the shell-crossing effect ends. We are left again with a gravitational pull, but with a slightly smaller mass $M_f$. Also, the initial value during this stage has to match the value of the previous state.
=======
